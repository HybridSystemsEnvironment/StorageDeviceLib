...
public class BroadcastSystemAv extends HybridSystem<BroadcastState>
{

	public StorageQueue storageQueue;
	public Network distributedNetwork;
	public Integer cycleTurn;
	public ArrayList<StorageConsistencyControl> controllers;

	public BroadcastSystemAv(Network distributed_network)
	{
		super(new BroadcastState());
		distributedNetwork = distributed_network;
		storageQueue = new StorageQueue();
		cycleTurn = 0;
		controllers = new ArrayList<StorageConsistencyControl>();
	}
	...
	{
		Node returnNode = controllers.get(cycleTurn).node;//distributedNetwork.getTopology().vertexSet()
	...
	public void nextTurn()
	{
		...
		//if (pendingUpdates())
		{
			...
			if (cycleTurn >= controllers.size())//distributedNetwork.getTopology().vertexSet().size() + 1)
			{
				cycleTurn = 0;
			}
		}
	...
	public boolean pendingUpdates()
	{
		...
		for (StorageConsistencyControl c : controllers)
		{
			updates = updates || c.localUpdateQueue.size() > 0.0;// .updates || n.getReceivingBuffer().size() > 0 || n.getTransmittingBuffer().size() > 0;
		}
		return updates;
	}
}
